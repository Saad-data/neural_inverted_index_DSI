...
def plot_pl_logs(log_dir, metric, save_dir):
    # Updated path for the log directory
    new_log_dir = './new_lightning_logs/version_6'

    # Ensure the new log directory exists
    if not os.path.exists(new_log_dir):
        print(f"The new log directory {new_log_dir} does not exist.")
        return
    ...
    # Find the event file in the new log directory
    try:
        event_file = next(file for file in os.listdir(new_log_dir) if 'events.out.tfevents' in file)
    except StopIteration:
        print(f"No event files found in directory {new_log_dir}")
        return
    full_path = os.path.join(new_log_dir, event_file)

    # Load the TensorBoard event file
    ea = event_accumulator.EventAccumulator(full_path)
    ea.Reload()

    # Extracting the scalar metric
